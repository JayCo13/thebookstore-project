...
- **Shipping (GoShip)** — `functions/_shared/goship.ts` is the only carrier client. GoShip is an **aggregator**: `POST /rates` returns what each of ~14 carriers (GHN, GHTK, J&T, Ninja Van, VNPost…) would charge for the route and `POST /shipments` books one of those quotes, so changing carrier is a runtime choice rather than another migration. The shop moved here after GHN's service and then VTP's API both proved unworkable — that history is why the schema is carrier-neutral.
  ...
  **Which carriers are used is a secret, not code.** `GOSHIP_CARRIERS` is an allowlist of GoShip short names (currently `shopee,vnp,jnt`); empty means take whatever is offered. If nothing on the list serves a route the quote falls back to the full list and logs a warning — refusing would leave the customer unable to order, which is worse than a non-preferred carrier. Dropping a carrier whose service has gone bad is now a `supabase secrets set`, which is the entire point of moving to an aggregator after doing this migration twice.
