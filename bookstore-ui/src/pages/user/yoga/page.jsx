...
export default function YogaPage() {
    ...
    // Fetch ALL yoga-related category IDs
    const fetchYogaCategories = async () => {
        try {
            ...
            // Find ALL categories that contain "yoga" in their name (case-insensitive)
            const yogaCats = categories.filter(cat =>
                cat.name.toLowerCase().includes('yoga')
            );

            if (yogaCats.length > 0) {
                const ids = yogaCats.map(cat => cat.category_id);
                setYogaCategoryIds(ids);
                console.log('Found Yoga categories:', yogaCats.map(c => `${c.name} (ID: ${c.category_id})`));
            } else {
                console.warn('No Yoga categories found in database');
            }
        } catch (err) {
            console.error('Error fetching yoga categories:', err);
        }
    };

    // Fetch yoga items from ALL yoga categories
    const fetchItems = async (filters = {}) => {
        if (yogaCategoryIds.length === 0) return;

        try {
            setLoading(true);
            setError(null);

            // Fetch items from each yoga category and combine them
            const allYogaItems = [];
            for (const categoryId of yogaCategoryIds) {
                const params = {
                    skip: 0,
                    limit: 100,
                ...
                allYogaItems.push(...items);
            }

            // Remove duplicates by stationery_id
            const uniqueItems = allYogaItems.filter((item, index, self) =>
                index === self.findIndex(t => t.stationery_id === item.stationery_id)
            );

            const formatted = uniqueItems.map(formatItemData);
            setItems(formatted);
        } catch (err) {
            console.error('Error fetching yoga items:', err);
            setError('Failed to load yoga items. Please try again later.');
        } finally {
            setLoading(false);
        }
    };
    ...
    // Fetch items when yoga categories are loaded or search changes (using debounced search)
    useEffect(() => {
        if (yogaCategoryIds.length > 0) {
            const filters = {};
            if (debouncedSearch.trim()) filters.search = debouncedSearch.trim();
            // Don't send filterMode to API - we'll sort on frontend
            fetchItems(filters);
        }
    }, [yogaCategoryIds, debouncedSearch]);  // Removed filterMode from dependencies
    ...
    useEffect(() => {
        setPage(1);
    }, [search, minPrice, maxPrice, sortBy, filterMode]);
        ...
        <section className="pt-24 pb-8 px-6 lg:px-8 max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6">
                <div>
                    ...
                    <p className="text-gray-600 mt-4 max-w-xl">
                        Khám phá và mua sắm các dụng cụ yoga chất lượng cao.
                    </p>
                </div>
            ...
            {loading && (
                <div className="flex justify-center items-center py-20">
                    ...
                    <span className="ml-3 text-gray-600">Đang tải dụng cụ yoga...</span>
                </div>
            )}
