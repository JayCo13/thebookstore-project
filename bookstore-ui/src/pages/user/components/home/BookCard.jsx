...
export default function BookCard({ book, onViewDetails, addToCart, addToWishlist }) {
  const images = useMemo(() => {
    ...
    if (Array.isArray(book.images)) {
      for (let i = 0; i < Math.min(3, book.images.length); i++) {
        if (book.images[i]) list.push(book.images[i]);
      }
    }
    if (list.length === 0 && book.cover) list.push(book.cover);
    return list;
  }, [book]);
  ...
  return (
    <div className="group rounded-lg overflow-hidden">
      <div className="relative w-full aspect-[2/3]">
        ...
        {book.isFreeShip && (
          <span
            ...
            style={{ marginTop: book.tag ? '36px' : '0' }}
          >
        ...
        {book.discountedPrice && (() => {
          const op = parsePrice(book.originalPrice);
          const dp = parsePrice(book.discountedPrice);
      ...
      <div className="pt-3">
        ...
        {book.author && <p className="text-sm text-gray-600 mt-1 truncate">Tác giả: {book.author}</p>}
        {/* Average rating */}
          ...
          <div className="mt-1 flex items-center">
            <div className="flex">
              {[0, 1, 2, 3, 4].map((i) => (
                <StarIcon
                  key={i}
                  className={`h-4 w-4 ${Math.round(book.averageRating) > i ? 'text-yellow-400' : 'text-gray-300'}`}
                />
              ))}
            </div>
            <span className="ml-2 text-xs text-gray-600">
              {(book.averageRating ?? 0).toFixed?.(1) ?? Number(book.averageRating || 0).toFixed(1)} / 5
              {typeof book.totalReviews !== 'undefined' && (
                <span className="ml-1">({book.totalReviews})</span>
              )}
            </span>
          </div>
        )}
        ...
        <div className="mt-2 flex items-center justify-between">
          {book.discountedPrice ? (
            <span className="flex items-center gap-2">
              <span className="text-red-600 font-semibold animate-pulse">{book.discountedPrice}</span>
